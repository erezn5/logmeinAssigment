...
public class FileUtil {
    ...
    public static boolean delete(File file){
        ...
        LoggerFactory.LOG.i("File was deleted =[%b]", success);
        return success;
    }
    ...
    public static boolean createNewFile(File file){
        if(file.exists()){
            LoggerFactory.LOG.i("file=[%s] already exist" , file.getAbsolutePath());
            return true;
        }else {
            try {
                if(file.createNewFile()){
                    LoggerFactory.LOG.i("file=[%s] created successfully" , file.getAbsolutePath());
                    return true;
                }else{
                    LoggerFactory.LOG.e("failed to create file=[%s]" , file.getAbsolutePath());
                }
            } catch (IOException e) {
                LoggerFactory.LOG.e(e ,"error occur creating file=[%s]" , file.getAbsolutePath());
            }
        }
        return false;
    }
    ...
    public static void writeToFile(String filePath , List<String> lines) {
        try(FileWriter fileWriter = new FileWriter(filePath);
            ...
            LoggerFactory.LOG.e(e ,"failed write to file=[%s]" , filePath);
        }
    }
    ...
    public static void serializeObj(File serObjToFile , Object serObj){
        if(FileUtil.createNewFile(serObjToFile)){
            try (FileOutputStream fos = new FileOutputStream(serObjToFile);
                ...
                LoggerFactory.LOG.e(e , "failed to serialize mock-device object=[%s] to file=[%s]" , serObj , serObjToFile.getAbsolutePath());
            }
        }
    }
    ...
    public static Object deserializeObj(File serObjToFile){
        if(serObjToFile.exists()){
            try (FileInputStream fis = new FileInputStream(serObjToFile);
                ...
                LoggerFactory.LOG.e(e , "failed to deserialize mock-device object from file=[%s]" , serObjToFile.getAbsolutePath());
            }
        }else{
            LoggerFactory.LOG.w("failed to deserialize mock-device object=[%s] file=[%s] don't exist" , serObjToFile.getAbsolutePath());
        }
        return null;
    }
